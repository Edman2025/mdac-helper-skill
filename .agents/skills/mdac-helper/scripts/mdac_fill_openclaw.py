...
Usage:
  mdac_fill_openclaw.py payload.json
...
- Dropdowns (Nationality, Sex, etc.) are not set here; set them manually or extend.
...
def build_fn(payload: dict) -> str:
  ...
  function setSelectById(id, want) {{
    ...
    // Special-case: region code "65" may have multiple entries; prefer SINGAPORE.
    if (!opt && wantStr === '65') {{
  ...
  // Wait for city options after state change
...
def main():
    if len(sys.argv) != 2:
        raise SystemExit("Usage: mdac_fill_openclaw.py payload.json")
